...
{
    glob->N = 2048;
    glob->counts = 100;//stop condition
    glob->Init = "rand";
    glob->maxnum = 15.0;//max number of element?what element?
    glob->w = 0.5;//relaxation factor
    glob->PRINT = 0;
}
...
Read_Options(int argc, char **argv)
{
	...
	if (**argv == '-')
	    ...
	    case 'u':
			...
			printf("           [-c counts] number of calculation time\n");
